...
Pebble.addEventListener("ready", function(e) {
  ...
  var pricing = localStorage.getItem("MEAL_PRICE");
  //console.log("openmensaID:", openmensaID);
  if (!openmensaID) {
    Pebble.sendAppMessage({
        "ERROR_MSG": "Please go into settings, configure openmensaID, and restart the app."
      }, function(e) {
  ...
  req.onload = function() {
    if (req.status >= 200 && req.status < 300) {
      ...
      console.log("XHR request failed: " + req.statusText);
    }
  };
...
Pebble.addEventListener("appmessage", function(e) {
  // If a date was selected, fetch meals and cache the data.
  if (e.payload.SELECTED_DATE) {
    var openmensaID = localStorage.getItem("openmensaID");
